...
router.post(
  ...
  body('roleName').isIn(['ADMIN', 'CASHIER']).withMessage('roleName must be ADMIN or CASHIER'),
...
router.patch(
  ...
  body('isActive').optional().isBoolean(),
  body('roleName').optional().isIn(['OWNER', 'ADMIN', 'CASHIER', 'CUSTOMER']),
  ...
  async (req: AuthRequest, res, next) => {
    try {
      ...
      if (target.role.name === 'OWNER') {
        if (req.body.isActive === false) throw new AppError(403, 'Owner account cannot be deactivated');
        if (req.body.roleName !== undefined && req.body.roleName !== 'OWNER') throw new AppError(403, 'Owner role cannot be changed');
      }
      ...
      if (roleName) {
        ...
        if (!role) throw new AppError(400, 'Invalid role');
        roleId = role.id;
        if (roleName === 'OWNER' && self.roleName !== 'OWNER') throw new AppError(403, 'Only owner can assign owner role');
      }
      ...
      if (req.body.password) {
        ...
        if (id !== self.id) update.forcePasswordChange = true;
      }
      if (req.body.forcePasswordChange !== undefined) update.forcePasswordChange = req.body.forcePasswordChange;
...
router.delete('/:id', async (req: AuthRequest, res, next) => {
  try {
    ...
    const target = await prisma.user.findUnique({ where: { id }, include: { role: true } });
    if (!target) throw new AppError(404, 'User not found');
    if (target.role.name === 'OWNER') throw new AppError(403, 'Cannot delete owner account');
    if (id === self.id) throw new AppError(403, 'Cannot delete your own account');
