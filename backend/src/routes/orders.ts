...
const PAYMENT_METHODS = ['GCASH', 'DEBIT_CARD', 'CASH_ON_DELIVERY'] as const;
...
router.post(
  ...
  body('zipCode').trim().notEmpty().withMessage('ZIP code is required').isLength({ max: 20 }),
  ...
  async (req: AuthRequest, res, next) => {
    try {
      ...
      const province = (req.body.province as string)?.trim();
      const zipCode = (req.body.zipCode as string)?.trim();
      const landmark = (req.body.landmark as string)?.trim() || null;
...
router.patch(
  '/:id/approve',
  authenticate,
  requireRoles('CASHIER', 'OWNER'),
  body('action').isIn(['approve', 'reject']),
  async (req: AuthRequest, res, next) => {
    try {
      const errors = validationResult(req);
      if (!errors.isEmpty()) throw new AppError(400, 'Invalid action');
      const id = Number(req.params.id);
