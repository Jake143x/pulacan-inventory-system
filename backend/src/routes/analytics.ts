...
async function getDailyRevenue(start: Date, end: Date): Promise<{ date: string; revenue: number }[]> {
...
function forecastRevenue(historical: { date: string; revenue: number }[], days: number): number {
...
router.get('/forecast-summary', async (_req, res, next) => {
  try {
    ...
    for (const i of inv) {
      ...
      if (daysLeft < DEFAULT_LEAD_DAYS || i.quantity === 0) stockOutCount++;
    }
...
router.get('/sales-forecast', async (req, res, next) => {
  try {
    ...
    if (range === '90') {
      ...
      if (endParsed < end) end.setTime(endParsed.getTime());
    } else {
      start = new Date();
    ...
    const forecasted: { date: string; revenue: number; forecast: boolean }[] = [];
...
router.get('/stock-depletion', async (_req, res, next) => {
  try {
    ...
    const result = inv.map((i) => {
      ...
      let riskLevel: 'Safe' | 'Low' | 'Critical' = 'Safe';
      if (i.quantity === 0 || daysLeft < 0) riskLevel = 'Critical';
      else if (daysLeft < 7) riskLevel = 'Critical';
      else if (daysLeft < 14) riskLevel = 'Low';
...
router.get('/reorder-recommendations', async (_req, res, next) => {
  try {
    ...
    for (const i of inv) {
      ...
      if (daysLeft >= 14 && i.quantity > 0) continue;
