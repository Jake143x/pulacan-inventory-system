 ...
 */
export async function runDemandPrediction(prisma: PrismaClient, daysAhead = 7) {
  ...
  const dailyByProduct = new Map<number, number[]>();
  for (let d = 0; d < 30; d++) {
  ...
  for (const product of products) {
    ...
    if (currentStock < threshold) riskOfStockout = 'HIGH';
    else if (currentStock < predictedDemand + threshold) riskOfStockout = 'MEDIUM';
