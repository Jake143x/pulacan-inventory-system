...
export function computeRiskLevel(quantity, avgDailyUnits, reorderQuantity) {
    if (quantity === 0)
        return 'Out of Stock';
...
export function shouldNotifyRiskChange(previousRisk, currentRisk) {
    if (previousRisk === currentRisk)
        return false;
    if (previousRisk === null) {
        return currentRisk !== 'Safe';
    }
    if (currentRisk === 'Out of Stock')
        return true;
    if (currentRisk === 'Critical' && (previousRisk === 'Safe' || previousRisk === 'Low'))
        return true;
    if (currentRisk === 'Low' && previousRisk === 'Safe')
        return true;
    if (currentRisk === 'Overstock')
        return true;
    if (currentRisk === 'No Data')
        return true;
    return false;
}
export function riskToNotificationType(risk) {
...
export function riskMessage(productName, risk) {
    switch (risk) {
        case 'Out of Stock':
...
export async function evaluateRiskAndNotify(prisma) {
    ...
    for (const i of inv) {
        ...
        const previousRisk = snapshot ? snapshot.riskLevel : null;
...
async function upsertSnapshot(prisma, productId, riskLevel) {
