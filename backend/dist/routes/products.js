...
router.get('/categories', async (req, res, next) => {
    try {
        const shop = req.query.shop === 'true';
        ...
        const categories = rows.map((r) => r.category).filter((c) => c != null);
...
router.post('/', body('name').trim().notEmpty(), body('sku').optional().trim(), body('category').optional().trim(), body('description').optional().trim(), body('specifications').optional().trim(), body('unitPrice').isFloat({ min: 0 }), body('imageUrl').optional().trim(), body('status').optional().isIn(['active', 'inactive']), body('initialQuantity').optional().isInt({ min: 0 }), body('lowStockThreshold').optional().isInt({ min: 0 }), body('reorderLevel').optional().isInt({ min: 0 }), body('reorderQuantity').optional().isInt({ min: 0 }), async (req, res, next) => {
    try {
...
router.post('/bulk-delete', body('productIds').isArray(), body('productIds.*').isInt(), async (req, res, next) => {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty())
            throw new AppError(400, errors.array()[0]?.msg ?? 'Validation failed');
...
router.patch('/bulk-category', body('productIds').isArray(), body('productIds.*').isInt(), body('category').trim().notEmpty(), async (req, res, next) => {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty())
            throw new AppError(400, errors.array()[0]?.msg ?? 'Validation failed');
...
router.patch('/:id', body('name').optional().trim().notEmpty(), body('sku').optional().trim(), body('category').optional().trim(), body('description').optional().trim(), body('specifications').optional().trim(), body('unitPrice').optional().isFloat({ min: 0 }), body('imageUrl').optional().trim(), body('status').optional().isIn(['active', 'inactive']), async (req, res, next) => {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty())
            throw new AppError(400, errors.array()[0]?.msg ?? 'Validation failed');
        ...
        if (sku !== undefined)
            update.sku = sku || null;
        if (cat !== undefined)
            update.category = cat || null;
        if (description !== undefined)
            update.description = description || null;
        if (specs !== undefined)
            update.specifications = specs || null;
        if (unitPrice !== undefined)
            update.unitPrice = Number(unitPrice);
        if (imgUrl !== undefined)
            update.imageUrl = imgUrl || null;
        if (st !== undefined)
            update.status = st === 'inactive' ? 'inactive' : 'active';
