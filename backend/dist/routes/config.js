...
router.put('/', body().isObject(), body('LOW_STOCK_THRESHOLD').optional().isString(), body(GCASH_QR_KEY).optional().isString(), async (req, res, next) => {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty())
            throw new AppError(400, 'Invalid config');
