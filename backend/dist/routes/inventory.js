...
router.get('/', async (req, res, next) => {
    try {
        ...
        if (category)
            withStatus = withStatus.filter((i) => i.product.category === category);
        if (search)
            withStatus = withStatus.filter((i) => (i.product.name?.toLowerCase().includes(search) || i.product.sku?.toLowerCase().includes(search)));
        if (minPrice != null && !Number.isNaN(minPrice))
            withStatus = withStatus.filter((i) => i.product.unitPrice >= minPrice);
        if (maxPrice != null && !Number.isNaN(maxPrice))
            withStatus = withStatus.filter((i) => i.product.unitPrice <= maxPrice);
        if (minQty != null && !Number.isNaN(minQty))
            withStatus = withStatus.filter((i) => i.quantity >= minQty);
        if (maxQty != null && !Number.isNaN(maxQty))
            withStatus = withStatus.filter((i) => i.quantity <= maxQty);
        const filtered = statusFilter === 'low' || statusFilter === 'low_stock'
...
router.patch('/:productId', body('quantity').optional().isInt({ min: 0 }), body('lowStockThreshold').optional().isInt({ min: 0 }), body('reorderLevel').optional().isInt({ min: 0 }), body('reorderQuantity').optional().isInt({ min: 0 }), body('movementNotes').optional().trim().isString(), async (req, res, next) => {
    try {
        ...
        if (lowStockThreshold !== undefined)
            update.lowStockThreshold = Number(lowStockThreshold);
        if (reorderLevel !== undefined)
            update.reorderLevel = Number(reorderLevel);
        if (reorderQuantity !== undefined)
            update.reorderQuantity = Number(reorderQuantity);
        ...
        const qtyDelta = inv.quantity - before.quantity;
        if (qtyDelta !== 0) {
        ...
        const reorderLevelVal = inv.reorderLevel ?? inv.lowStockThreshold;
...
router.post('/bulk-adjust', body('items').isArray(), body('items.*.productId').isInt(), body('items.*.quantityDelta').isInt(), body('items.*.notes').optional().trim().isString(), async (req, res, next) => {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty())
            throw new AppError(400, errors.array()[0]?.msg ?? 'Validation failed');
