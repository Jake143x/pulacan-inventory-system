...
router.post('/register', body('email').isEmail().normalizeEmail(), body('password').isLength({ min: 8, max: 128 }), body('fullName').trim().notEmpty().isLength({ min: 2, max: 100 }), body('contactNumber').optional().trim().isLength({ max: 30 }), async (req, res, next) => {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty())
            throw new AppError(400, errors.array()[0]?.msg ?? 'Validation failed');
        ...
        const contactNumberVal = contactNumber?.trim() || null;
        const now = new Date();
...
router.get('/me', authenticate, async (req, res, next) => {
    try {
        ...
        const baseSelect = { id: true, email: true, fullName: true, isActive: true, lastLoginAt: true, createdAt: true, role: { select: { name: true } } };
        let user;
        try {
            ...
            user = await prisma.user.findUnique({
                where: { id: userId },
                select: baseSelect,
            }).then((u) => (u ? { ...u, contactNumber: null, profilePictureUrl: null } : null));
        }
        if (!user)
...
router.patch('/me', authenticate, body('fullName').optional().trim().notEmpty().isLength({ min: 2, max: 100 }), body('email').optional().isEmail().normalizeEmail(), body('contactNumber').optional().trim().isLength({ max: 30 }), body('profilePictureUrl').optional().trim().isLength({ max: 500 }), body('password').optional().isLength({ min: 8, max: 128 }), async (req, res, next) => {
    try {
        const errors = validationResult(req);
        if (!errors.isEmpty())
            throw new AppError(400, errors.array()[0]?.msg ?? 'Validation failed');
        ...
        if (req.body.contactNumber !== undefined)
            update.contactNumber = req.body.contactNumber?.trim() || null;
        if (req.body.profilePictureUrl !== undefined)
            update.profilePictureUrl = req.body.profilePictureUrl?.trim() || null;
